...
int bicubic_param_matrix[16][16] = {
    ...
    {-3 ,6  ,-3 ,0  ,9  ,-18,9  ,9  ,-9 ,18 ,-9 ,0  ,3  ,-6 ,3  ,0  },
...
int bicubic_spline_param_matrix[16][16]=
    {
        ...
        {0, 8, 0, 0, 32, -4, -24, 0, -20, -4, 16, 0, 0, 0, -4, 0}, 
        {0, -64, 40, 0, -64, 96, -68, 24, 40, -68, 68, -16, 0, 24, -16, 4}, 
        {0, 40, -24, 0, 32, -52, 52, -24, -20, 40, -52, 16, 0, -16, 12, -4}, 
        {0, -8, 0, 0, 0, 24, 0, 0, 0, -24, 0, 0, 0, 8, 0, 0}, 
        {0, -4, 0, 0, -20, 0, 16, 0, 12, 4, -12, 0, 0, 0, 4, 0}, 
...
unsigned char interpolation_compute_spline(float x, float y){
    return (unsigned char)max(min(((coefficient[0] + coefficient[1]*x + coefficient[2]*x*x + coefficient[3]*x*x*x +
            coefficient[4]*y + coefficient[5]*x*y + coefficient[6]*x*x*y+ coefficient[7]*x*x*x*y +
            coefficient[8]*y*y + coefficient[9]*x*y*y + coefficient[10]*x*x*y*y+ coefficient[11]*x*x*x*y*y +
            coefficient[12]*y*y*y + coefficient[13]*x*y*y*y + coefficient[14]*x*x*y*y*y+ coefficient[15]*x*x*x*y*y*y
            )/16),255),0);
...
*((unsigned char *)(&pixel_old[(i-1)+((j+2)*960)])+rgb), *((unsigned char *)(&pixel_old[(i)+((j+2)*960)])+ rgb), *((unsigned char *)(&pixel_old[(i+1)+((j+2)*960)])+ rgb), *((unsigned char *)(&pixel_old[(i+2)+((j+2)*960)])+ rgb)
                ...
                for(int y=2;y<=2;y++){
                    ...
                    if(i==960-3){
                        ...
                        int x_=4;
                        *(((unsigned char *)(&pixel_new[(i*4+x_+ 2)+((j*4+y_+ 2)*3840)])) + rgb) = interpolation_compute_spline(0.25*x_,0.25*y_);
                        }
                    }
                    if(j==540-3){
    ...
    for (int rgb=0; rgb<3; rgb++){
        ...
        int j=0;        // first line
            short b=*((unsigned char *)(&pixel_new[(i*4+2)+((j*4+2)*3840)])+rgb);
            short k=*((unsigned char *)(&pixel_new[((i+1)*4+2)+((j*4+2)*3840)])+rgb) - b;
            for(int x=0;x<=3;x++){
                int y=0;
                *(((unsigned char *)(&pixel_new[(i*4+x+2)+((j*4+y+2)*3840)])) + rgb) = \
                max(min(((k*x)/4) + b, 255),0);
            }
        }
        for (int i=0; i< (960-1); i++){
        ...
        for (int j=0; j<(540-1); j++){      // (i,j) represent location of orgin img.
            ...
            short b=*((unsigned char *)(&pixel_new[(i*4+2)+((j*4+2)*3840)])+rgb);
            short k=*((unsigned char *)(&pixel_new[((i)*4+2)+(((j+1)*4+2)*3840)])+rgb) - b;
            for(int y=0;y<=3;y++){
                int x=0;
                *(((unsigned char *)(&pixel_new[(i*4+x+2)+((j*4+y+2)*3840)])) + rgb) = \
                max(min(((k*y)/4) + b, 255),0);
            }
        }
        for (int j=0; j<(540-1); j++){      // (i,j) represent location of orgin img.
        ...
        for (int i=0; i<(960); i++){      // (i,j) represent location of orgin img.
            int j=0;        //first line space
            for(int x=0;x<=3;x++){
            short b=*((unsigned char *)(&pixel_new[(i*4 +2 + x)+((j*4+2)*3840)])+rgb);
            short k=*((unsigned char *)(&pixel_new[(i*4 +2 + x)+(((j+1)*4 +2)*3840)])+rgb) - b;
        ...
        for (int j=0; j<(540); j++){      // (i,j) represent location of orgin img.
            int i=0;        //first column space
            for(int y=0;y<=3;y++){
            short b=*((unsigned char *)(&pixel_new[(i*4+2)+((j*4 + y)*3840)])+rgb);
            short k=*((unsigned char *)(&pixel_new[((i+1)*4 +2)+((j*4 + y)*3840)])+rgb) - b;
                
                for(int x=-2;x<=3;x++){
                *(((unsigned char *)(&pixel_new[(i*4+x+2)+((j*4+y)*3840)])) + rgb) = \
                max(min(((k*x)/4) + b, 255),0);
                }
            }
        }
